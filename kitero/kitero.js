...
function hideBasedOnCheckbox(checkbox){
    const checkPar = checkbox.parentElement
    const checkVal = checkbox.checked
    if (checkVal == true){
        makeInvisibleBasedOnid(checkPar, "no")
        makeVisibleBasedOnId(checkPar, "ferfi")
    }
    else{
        makeInvisibleBasedOnid(checkPar, "ferfi")
        makeVisibleBasedOnId(checkPar, "no")
    }
}
...
function hideBasedOnSelected(dropdownList){
    const parVal = dropdownList.value
    const dropPar = dropdownList.parentElement
    const cardCl = dropdownList.querySelectorAll(".card")

    for(const i of cardCl){
        i.classList.add("hide")
    }
    makeInvisibleBasedOnid(dropPar, parVal)
}